...
contain the maximum number of nodes (i.e 2**(i-1) at the level i, note that we 
start counting the levels from 1 at the root). In level H, which may contain 
...
nodes come first, the leaves come second, and empty successors (the nil's 
...
/*
See they have the Huffman-problem: p50_huffman_code.pl BEFORE they ask you
to construct a heap?

Wherefore.

Okay, so I have a solution for ya:
*/
...
% ... but seriously: see the p50_huffman_code for examples of heap-use.
